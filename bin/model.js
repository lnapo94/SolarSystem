class Model {
    ...
    constructor(gl, data) {
        ...
        // load stuff
        this.name = data.name;
        this.texture = this.loadTexture(document.getElementById(data.texture), false);
        this.shader = new Shader(this.gl, data.vShaderURL, data.fShaderURL, false);
        this.loadMesh(data.mesh);

        //setup initial position contained in data
        this.transform = new Transform();
        this.transform.setScale(data.scale.X, data.scale.Y, data.scale.Z);
        this.transform.setPosition(data.position.X, data.position.Y, data.position.Z);

        // setup stuff related to object movement every tick
        // translation must be function?
        this.onTimePassingNewPositions = data.motion.translation;
        this.rotation = data.motion.rotation;
    }

    loadMesh(res){
        let meshResource = new OBJ.Mesh(res);
        this.setupBuffers(
    ...
    onTimePassing(deltaTime, actualPosition){
        this.transform.addRotation(
            deltaTime * this.rotation.X,
            deltaTime * this.rotation.Y,
            deltaTime * this.rotation.Z);
        this.transform.setPosition(
            this.onTimePassingNewPositions.X(actualPosition),
            this.onTimePassingNewPositions.Y(actualPosition),
            this.onTimePassingNewPositions.Z(actualPosition)
        );
