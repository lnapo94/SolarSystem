class Mesh {
    constructor(gl, noCulling = false, doBlending = false) {
        this.gl = gl;
        this.noCulling = noCulling;
        this.doBlending = doBlending;
    }

    setupBuffer(vertices, indices, normals, uvs, verticesLength = 3) {
    ...
    renderMesh() {
        ...
        // Setup the option for drawing
        if(this.noCulling)
            this.gl.disable(this.gl.CULL_FACE);
        if(this.doBlending)
            this.gl.enable(this.gl.BLEND);

        // Select the proper way to draw the mesh
        if(this.indexCount)
